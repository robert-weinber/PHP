...
<div id="content">
		...
		if ($stmtA = $mysqli->prepare("SELECT id, name, public, generatedname, description FROM downloads ORDER BY ordering")){
		$stmtA->execute();    
        $stmtA->store_result();
        $stmtA->bind_result($id, $name, $public, $generatedname, $description);
         while($stmtA->fetch()){
